...
struct proc_dir_entry *hx_proc_ctp_openshort_test_file = NULL;

char hx_touch_test_time[40];  //bug696971,fangzhihua.wt,add, 20220311,for TP openshort test result save

int himax_factory_test(struct seq_file *s, void *v)
{
	int val = 0;
	size_t ret = 0;
	...
	memset(hx_touch_test_time,'\0',sizeof(hx_touch_test_time));
	do_gettimeofday(&ktv);
	ktv.tv_sec -= sys_tz.tz_minuteswest * 60;
	rtc_time_to_tm(ktv.tv_sec, &rtc_tm);
	snprintf(hx_touch_test_time, sizeof(hx_touch_test_time),
...
{
    if(himax_factory_test(m,v) >  0)	//P220312-01614 pengzhenhua1.wt,modify,20220402,ATO test TP openshort always pass 
        seq_printf(m, "%s\n", "result=0");
    else
        seq_printf(m, "%s\n", "result=1");
    return 0;
...
{
	hx_proc_touchscreen_dir = proc_mkdir(PROC_TOUCHSCREEN_FOLDER , NULL);
	if (hx_proc_touchscreen_dir == NULL)  {
		I(KERN_ERR "%s: proc_touchpanel_dir file create failed!\n", __func__);
	}else{
		hx_proc_ctp_openshort_test_file = proc_create(PROC_CTP_OPENSHORT_TEST_FILE,
				(S_IWUSR | S_IRUGO), hx_proc_touchscreen_dir, &hx_proc_ctp_openshort_test_fops);
		if(hx_proc_ctp_openshort_test_file ==NULL){
			I(KERN_ERR "%s: proc_ctp_openshort_test_file create failed!\n", __func__);
			remove_proc_entry(PROC_TOUCHSCREEN_FOLDER, NULL);
		}else{
			I("%s:  proc_create PROC_CTP_OPENSHORT_TEST_FILE success",__func__);
		}
